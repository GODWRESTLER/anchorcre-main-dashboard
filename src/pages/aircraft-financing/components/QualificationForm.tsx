        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          >
            ...
            <p className="text-xl text-slate-600 mb-8">
              AI-powered matching with specialized aviation lenders. Competitive rates and flexible terms for aircraft financing.
            </p>
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-6">
            Get Fix & Flip Loan Quote in{' '}
            <span className="text-blue-500">2 Minutes</span>
          </h2>
          <p className="text-xl text-slate-600 mb-8">
            AI-powered matching with specialized fix & flip lenders. Fast approvals for real estate investment projects.
          </p>
          ...
          <div className="flex items-center justify-center space-x-4 mb-8">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-slate-600">Fix & Flip Specialists</span>
            </div>
          </div>
        </motion.div>
          ...
          <form onSubmit={handleSubmit}>
            {currentStep === 1 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h3 className="text-2xl font-bold text-slate-900 mb-6">Financing Details</h3>
                
                <div>
                  <label className="block text-slate-700 font-semibold mb-3">Financing Type</label>
                  <div className="grid grid-cols-1 gap-3">
                    {['Aircraft Purchase Loan', 'Aircraft Lease', 'Refinance Existing Aircraft', 'Partnership/Fractional'].map((type) => (
                      <button
                        key={type}
                        type="button"
                        onClick={() => handleInputChange('financingType', type)}
                ...
                <div>
                  <label className="block text-slate-700 font-semibold mb-3">Aircraft Type</label>
                  <div className="grid grid-cols-2 gap-3">
                    {['Single Engine', 'Twin Engine', 'Turboprop', 'Light Jet', 'Mid-Size Jet', 'Heavy Jet'].map((type) => (
                      <button
                        key={type}
                        type="button"
                        onClick={() => handleInputChange('aircraftType', type)}
            ...
            {currentStep === 2 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h3 className="text-2xl font-bold text-slate-900 mb-6">Aircraft Details</h3>
                
                <div>
                  <label className="block text-slate-700 font-semibold mb-3">Aircraft Year</label>
                  <input
                    type="text"
                    value={formData.aircraftYear}
                    onChange={(e) => handleInputChange('aircraftYear', e.target.value)}
                    className="w-full p-4 border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:outline-none"
                ...
                <div>
                  <label className="block text-slate-700 font-semibold mb-3">Tail Number (if known)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-slate-700 font-semibold mb-3">Aircraft Location</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {['ASAP (< 2 weeks)', '2-4 weeks', '1-2 months', 'Planning stage (researching)'].map((timeline) => (
                      <button
                        key={timeline}
                        type="button"
            ...
            {currentStep === 3 && (
              ...
              >
                ...
                <div>
                  <label className="block text-slate-700 font-semibold mb-2">Entity that owns/will own the aircraft</label>
                  <input
                    type="text"
                    value={formData.entity}
                    onChange={(e) => handleInputChange('entity', e.target.value)}
                    className="w-full p-3 sm:p-4 border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:outline-none text-base"
                    placeholder="e.g., ABC Aviation LLC, Individual, Trust"
                  />
                </div>
